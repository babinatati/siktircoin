...
#if defined(NDEBUG)
# error "Siktircoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6197a051d9fb2a67ed3859b83056d574ffc035b1987be71da1de585151dedd60");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Siktircoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Siktircoin Signed Message:\n";
...
{
    // Siktircoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Siktircoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Siktircoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Siktircoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Siktircoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Siktircoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc810e0111df304e95e2a003a566dd9c7dd6c9b8a933049e145d1a3949a6d4f5d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Siktircoin is lunched for the first time 14.03.18";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042634293e962d59e75d4937cf1702b093b4fc077358aa9eef629d2a1a701a0870a6405bb66a3b131ec56de116877f90c1433117e2ffbd387e290724f2a8386ba9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520978069;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085881656;

        if (fTestNet)
        {
            block.nTime    = 1520978043;
            block.nNonce   = 385562733;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5d0779df013c2ad4d166b98d8932be3605df1acc9844d3ebcbd225845304a0fd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc5, 0xbc, 0xd6 }; // Siktircoin: increase each by adding random values.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SiktircoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SiktircoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SiktircoinMiner(CWallet *pwallet)
{
    printf("SiktircoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("siktircoin-miner");
    ...
    try { loop {
        ...
        printf("Running SiktircoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SiktircoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SiktircoinMiner, pwallet));
}
